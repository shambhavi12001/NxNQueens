...
public class DAC {
	...
	private ArrayList<Node> dacList = new ArrayList<Node>();
	private String initialPos;
	private int backTrackDirectional;
	...
	public int getBackTrackDirectional() {
		return backTrackDirectional;
	}
	...
	public void run_dac(String initialPos) {
		while(true) {
			dac(initialPos);
			if(positions.size() == 8) {
				solutions++;
				this.backTrackList.add(this.backTrackDirectional);
			}
			if(squares.get(1).getDomain().size()==0 && this.calculateBreakLoop() == 8) {
				break;
			}
	...
	public void printSolutions() {
		for(Node pos : positions) {
			this.solutionList.get(solutionList.size() - 1).add(pos);
		}
			
		
	}

	public void dac(String initialPos) {
		...
		while(positions.size() < 8) {
			...
			for(int j = positions.get(positions.size()-1).getRow() + 1; j <= 8; ++j) {
				
				for(int column : squares.get(j-1).getDomain()) {
					numNodes = 0;
					for(Node check : nodes) {
						if((!(check.getRow() == j && check.getVariable() == column)) &&
								(check.getRow() == j || check.getVariable() == column || 
								(Math.abs(check.getRow() - j) == Math.abs(check.getVariable() - column)))) {
							this.dacList.add(new Node(check.getRow(), check.getVariable()));
							
						}
					}
					for(int k = j + 1; k <= 8; ++k) {
						for(Node dacListNode : dacList) {
							if(k == dacListNode.getRow()) {
								numNodes ++;
							}
						}
						for(ArrayList<Node> row : deleted){
							for(Node col : row) {
								if(col.getRow() == k) {
									numNodes ++;
								}
						...
						if(numNodes == 8) {
							deleted.get(deleted.size() - 1).add(new Node(j, column));
							i = 0;
							for(Node n : nodes) {
								if(n.getRow() == j && n.getVariable() == column) {
									break;
								}
								i++;
							}
				...
				for(Node deletedNode : deleted.get(deleted.size() -1)) {
					if(squares.get(deletedNode.getRow() - 1).getDomain().contains(deletedNode.getVariable())) {
						squares.get(deletedNode.getRow() - 1).getDomain().removeAll(Collections.singleton(deletedNode.getVariable()));
					}
				}
